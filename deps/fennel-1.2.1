...
package.preload["fennel.binary"] = package.preload["fennel.binary"] or function(...)
  ...
  local _local_769_ = require("fennel.utils")
  local warn = _local_769_["warn"]
  local copy = _local_769_["copy"]
  ...
  local function execute(cmd)
    local _770_ = os.execute(cmd)
    if (_770_ == 0) then
      return true
    elseif (_770_ == true) then
      return true
    else
      return nil
    end
  ...
  local function compile_binary(lua_c_path, executable_name, static_lua, lua_include_dir, native)
    ...
    local _780_
    do
      local _779_ = shellout((cc .. " -dumpmachine"))
      if (nil ~= _779_) then
        _780_ = _779_:match("mingw")
      else
        _780_ = _779_
      end
    end
    ...
    compile_command = {cc, "-Os", lua_c_path, table.concat(native, " "), static_lua, rdynamic, "-lm", _783_, "-o", (executable_name .. bin_extension), "-I", lua_include_dir, os.getenv("CC_OPTS")}
  ...
  local function native_path_3f(path)
    ...
    if (version_extension and (version_extension ~= "") and not version_extension:match("%.%d+")) then
      ...
      local _788_ = extension
      if (_788_ == "a") then
        return path
      elseif (_788_ == "o") then
        return path
      elseif (_788_ == "so") then
        return path
      elseif (_788_ == "dylib") then
        return path
      elseif true then
        local _ = _788_
        return false
  ...
  local function compile(filename, executable_name, static_lua, lua_include_dir, options, args)
    local _let_795_ = extract_native_args(args)
    local modules = _let_795_["modules"]
    local libraries = _let_795_["libraries"]
    local rename_modules = _let_795_["rename-modules"]
...
local function dosafely(f, ...)
  ...
  result = pack(xpcall(_742_, fennel.traceback))
...
local function allow_globals(global_names, globals)
  if (global_names == "*") then
    ...
    do
      ...
      options.allowedGlobals = tbl_14_auto
    end
...
do
  ...
  while (arg[i] and not options["ignore-options"]) do
    local _750_ = arg[i]
    if (_750_ == "--no-searcher") then
      options["no-searcher"] = true
      table.remove(arg, i)
    elseif (_750_ == "--indent") then
      ...
      table.remove(arg, i)
    elseif (_750_ == "--add-package-path") then
      local entry = table.remove(arg, (i + 1))
      package.path = (entry .. ";" .. package.path)
      table.remove(arg, i)
    elseif (_750_ == "--add-fennel-path") then
      local entry = table.remove(arg, (i + 1))
      fennel.path = (entry .. ";" .. fennel.path)
      table.remove(arg, i)
    elseif (_750_ == "--add-macro-path") then
      local entry = table.remove(arg, (i + 1))
      fennel["macro-path"] = (entry .. ";" .. fennel["macro-path"])
      table.remove(arg, i)
    elseif (_750_ == "--load") then
      handle_load(i)
    elseif (_750_ == "-l") then
      handle_load(i)
    elseif (_750_ == "--no-fennelrc") then
      options.fennelrc = false
      table.remove(arg, i)
    elseif (_750_ == "--correlate") then
      options.correlate = true
      table.remove(arg, i)
    elseif (_750_ == "--check-unused-locals") then
      options.checkUnusedLocals = true
      table.remove(arg, i)
    elseif (_750_ == "--globals") then
      allow_globals(table.remove(arg, (i + 1)), _G)
      table.remove(arg, i)
    elseif (_750_ == "--globals-only") then
      allow_globals(table.remove(arg, (i + 1)), {})
      table.remove(arg, i)
    elseif (_750_ == "--require-as-include") then
      options.requireAsInclude = true
      table.remove(arg, i)
    elseif (_750_ == "--skip-include") then
      ...
      table.remove(arg, i)
    elseif (_750_ == "--use-bit-lib") then
      options.useBitLib = true
      table.remove(arg, i)
    elseif (_750_ == "--metadata") then
      options.useMetadata = true
      table.remove(arg, i)
    elseif (_750_ == "--no-metadata") then
      options.useMetadata = false
      table.remove(arg, i)
    elseif (_750_ == "--no-compiler-sandbox") then
      options["compiler-env"] = _G
      table.remove(arg, i)
    elseif (_750_ == "--raw-errors") then
      options.unfriendly = true
      table.remove(arg, i)
    elseif (_750_ == "--plugin") then
...
local function eval(form)
  local _760_
  if (form == "-") then
    _760_ = (io.stdin):read("*a")
  else
    _760_ = form
  end
  return print(dosafely(fennel.eval, _760_, options))
end
local function compile(files)
  for _, filename in ipairs(files) do
    ...
    do
      local _763_, _764_ = nil, nil
      local function _765_()
        return fennel["compile-string"](f:read("*a"), options)
      end
      _763_, _764_ = xpcall(_765_, fennel.traceback)
      if ((_763_ == true) and (nil ~= _764_)) then
        local val = _764_
        print(val)
      elseif (true and (nil ~= _764_)) then
        local _0 = _763_
        local msg = _764_
...
if ((_G.type(_767_) == "table") and _768_(...)) then
  return repl()
elseif ((_G.type(_767_) == "table") and ((_767_)[1] == "--repl")) then
  return repl()
elseif ((_G.type(_767_) == "table") and ((_767_)[1] == "--compile")) then
  local files = {select(2, (table.unpack or _G.unpack)(_767_))}
  return compile(files)
elseif ((_G.type(_767_) == "table") and ((_767_)[1] == "-c")) then
  local files = {select(2, (table.unpack or _G.unpack)(_767_))}
  return compile(files)
elseif ((_G.type(_767_) == "table") and ((_767_)[1] == "--compile-binary") and (nil ~= (_767_)[2]) and (nil ~= (_767_)[3]) and (nil ~= (_767_)[4]) and (nil ~= (_767_)[5])) then
  local filename = (_767_)[2]
  local out = (_767_)[3]
  local static_lua = (_767_)[4]
  local lua_include_dir = (_767_)[5]
  local args = {select(6, (table.unpack or _G.unpack)(_767_))}
  local bin = require("fennel.binary")
  options.filename = filename
  options.requireAsInclude = true
  return bin.compile(filename, out, static_lua, lua_include_dir, options, args)
elseif ((_G.type(_767_) == "table") and ((_767_)[1] == "--compile-binary")) then
  return print((require("fennel.binary")).help)
elseif ((_G.type(_767_) == "table") and ((_767_)[1] == "--eval") and (nil ~= (_767_)[2])) then
  local form = (_767_)[2]
  return eval(form)
elseif ((_G.type(_767_) == "table") and ((_767_)[1] == "-e") and (nil ~= (_767_)[2])) then
  local form = (_767_)[2]
  return eval(form)
else
  ...
  if (((_G.type(_767_) == "table") and (nil ~= (_767_)[1])) and _796_(...)) then
    local a = (_767_)[1]
    return print(fennel["runtime-version"]())
  elseif ((_G.type(_767_) == "table") and ((_767_)[1] == "--help")) then
    return print(help)
  elseif ((_G.type(_767_) == "table") and ((_767_)[1] == "-h")) then
    return print(help)
  elseif ((_G.type(_767_) == "table") and ((_767_)[1] == "-")) then
    local args = {select(2, (table.unpack or _G.unpack)(_767_))}
    return dosafely(fennel.eval, (io.stdin):read("*a"))
  elseif ((_G.type(_767_) == "table") and (nil ~= (_767_)[1])) then
    local filename = (_767_)[1]
    local args = {select(2, (table.unpack or _G.unpack)(_767_))}
    arg[-2] = arg[-1]
    arg[-1] = arg[0]
